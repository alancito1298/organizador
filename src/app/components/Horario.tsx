...
export default function Horario() {
  ...
  // =====================
  return (
    <div className="min-h-screen bg-[#f6f4ff] p-4 md:p-8">
     <Navbar></Navbar>
      <div className="max-w-7xl mx-auto mt-10">
        ...
        <div className="flex flex-col gap-5 md:hidden">
          {DIAS.map((dia) => (
            <div
              key={dia}
              className="bg-white  border border-violet-100 shadow-sm p-5"
            >
              ...
              <div className="flex flex-row items-center w-full justify-center gap-3 ">
                {filas.map((fila, filaIndex) => (
                  <div
                    key={filaIndex}
                    className=" border-violet-100 flex gap-4 flex-row items-center bg-red-500"
                  >
                    <input
                      type="text"
                      placeholder="08:00"
                      value={fila.hora}
                      onChange={(e) =>
                        actualizarHora(filaIndex, e.target.value)
                      }
        ...
        <div className="hidden md:block overflow-x-auto">
          <div className="grid grid-cols-6 gap-4 min-w-[1100px]">
            ...
            <div className="flex flex-col gap-4">
              <div className="h-16 flex items-center justify-center rounded-2xl bg-violet-900 text-white font-semibold">
                Hora
              </div>
              ...
              {filas.map((fila, filaIndex) => (
                <div
                  key={filaIndex}
                  className="bg-white border border-violet-100 rounded-2xl p-3 shadow-sm"
                >
                  <input
                    type="text"
                    placeholder="08:00"
                    ...
                    className="w-full bg-transparent outline-none text-center font-semibold text-violet-900"
                  />
                </div>
              ))}
            </div>
            ...
            {DIAS.map((dia) => (
              <div key={dia} className="flex flex-col gap-4">
                ...
                {/* Header día */}
                <div className="h-16 flex items-center justify-center rounded-2xl bg-white border border-violet-100 shadow-sm">
                ...
                {filas.map((fila, filaIndex) => (
                  <div
                    key={filaIndex}
                    className="bg-white border border-violet-100 rounded-2xl p-3 shadow-sm hover:border-violet-300 transition"
                  >
                    <textarea
                      placeholder="Materia / Nota"
                      value={fila.celdas[dia].descripcion}
                      ...
                      className="w-full min-h-[120px] resize-none outline-none text-sm text-violet-900"
                    />
                  </div>
                ))}
              </div>
